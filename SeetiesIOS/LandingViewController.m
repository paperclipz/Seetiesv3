...
//    NSString *CheckStirng = @"abcccc @";
//    if ([CheckStirng rangeOfString:@" @"].location == NSNotFound) {
//        NSLog(@"no ?");
//    } else {
//        NSLog(@"with @ ?");
//    }
    //AdrianeText-BoldItalic
    
    
  //  [NSThread detachNewThreadSelector:@selector(GetExternalIPAddress) toTarget:self withObject:nil];
}
    ...
    if (iPURL) {
        ...
        if (!error) {
            ...
            while ([theScanner isAtEnd] == NO) {
                ...
                ExternalIPAddress =[ipItemsArray objectAtIndex: ++an_Integer];
            }
            NSLog(@"ExternalIPAddress : %@",ExternalIPAddress);
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:ExternalIPAddress forKey:@"ExternalIPAddress"];
            [defaults synchronize];
          //  [self GetIpAddress];
            [NSThread detachNewThreadSelector:@selector(GetIpAddress) toTarget:self withObject:nil];
        } else {
            NSLog(@"Oops... g %ld, %@",
                  (long)[error code],
...
-(void)GetIpAddress{
    NSLog(@"GetID Address Work ?");
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *ExternalIPAddress = [defaults objectForKey:@"ExternalIPAddress"];
    NSLog(@"ExternalIPAddress is %@",ExternalIPAddress);
...
//                 NSJSONReadingMutableContainers error:&error];
    ...
    }else
    {
        ...
        if (!myError) {
            NSString *GetLat = [[NSString alloc]initWithFormat:@"%@",[res objectForKey:@"latitude"]];
            NSLog(@"GetLat is %@",GetLat);
            
            NSString *GetLang = [[NSString alloc]initWithFormat:@"%@",[res objectForKey:@"longitude"]];
            NSLog(@"GetLang is %@",GetLang);
            
            //  NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:GetLat forKey:@"GetLat"];
            [defaults setObject:GetLang forKey:@"GetLang"];
            [defaults synchronize];
        }else {
            NSLog(@"Oops... g %ld, %@",
                  (long)[myError code],
...
//    [body appendData:[[NSString stringWithFormat:@"%@",UserEmail] dataUsingEncoding:NSUTF8StringEncoding]];
//    [body appendData:[@"\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
//    
//    //parameter second
//    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
...
//    //Attaching the content to be posted ( ParameterSecond )
//    [body appendData:[[NSString stringWithFormat:@"%@",Userdob] dataUsingEncoding:NSUTF8StringEncoding]];
//    [body appendData:[@"\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
//    
//    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
//    //Attaching the key name @"parameter_second" to the post body
...
//    //Attaching the content to be posted ( ParameterSecond )
//    [body appendData:[[NSString stringWithFormat:@"%@",UserGender] dataUsingEncoding:NSUTF8StringEncoding]];
//    [body appendData:[@"\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
...
//                [PTnCView GetFBLogin:@"YES"];
    ...
    //get all languages
      ...
      //  NSLog(@"LanguageCode_Array is %@",LanguageCode_Array);
        ...
        [self GetALlCategory];
    }else if(connection == theConnection_GetIPAddress){
        NSString *GetData = [[NSString alloc] initWithBytes: [webData mutableBytes] length:[webData length] encoding:NSUTF8StringEncoding];
